  ...
  function resolveUrl(href) {
    ...
    if (!href.includes('/')) {
      ...
      console.log('Resolving relative URL:', {
        ...
        currentPath,
        currentDir,
        proposed: origin + currentDir + href
      });
      
      return origin + currentDir + href;
    }
    
    // For other relative paths, join with the base path
    let resolvedUrl = origin + basePath;
    ...
    // Join the base path and href, avoiding double slashes
    if (resolvedUrl.endsWith('/') && href.startsWith('/')) {
      resolvedUrl += href.substring(1);
    } else if (!resolvedUrl.endsWith('/') && !href.startsWith('/')) {
      resolvedUrl += '/' + href;
    } else {
      resolvedUrl += href;
    }
